...
class GAT(torch.nn.Module):
  def __init__(self, model_checkpoint, dropout_p, patterns_len):
    ...
    self.feat_extr = nn.ModuleList([
                       nngm.GATConv(in_channels=6, out_channels=16, heads=32, concat=True, dropout=.5),
                       nngm.GATConv(in_channels=16*32, out_channels=16, heads=32, concat=True, dropout=.5),
                       nngm.GATConv(in_channels=16*32, out_channels=16, heads=32, concat=True, dropout=.5),
                       nngm.GATConv(in_channels=16*32, out_channels=16, heads=32, concat=True, dropout=.5),
                       nngm.GATConv(in_channels=16*32, out_channels=16, heads=32, concat=False, dropout=.5),
    ])
    
    self.head = nn.Sequential(
                       nn.Linear(16, 64),
                       nn.Dropout(dropout_p),
                       nn.Linear(64, 2**patterns_len),
    )
    self.initialize_weights(self.feat_extr)
    self.initialize_weights(self.head)
    self.patterns_len = patterns_len

  def forward(self, data, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')): #x, edge_index=None):
    #print(edge_index.shape, edge_index.dtype, x.shape, x.dtype,)
    x, edge_index = data
    x = x.to(device)
    edge_index = edge_index.to(device)
    #print(f"models.py:  x-> {x.shape}, edge_index-> {edge_index.shape}")
    for layer in self.feat_extr:
      #print(f"layer {layer}, layer dtype: {list(layer.parameters())[0].dtype}, shape: {x.shape}, dimensions: {x.dim()}")
      x = layer(x, edge_index)
    x = self.head(x)[0] #.mean(0)
    #print(x.shape.sum(0))
    #print(x.shape)
