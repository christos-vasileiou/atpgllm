...
#SBATCH --time=10:00:00                # Time limit (24 hours, adjust as needed)
#SBATCH --nodes=1                      # Request 1 node
#SBATCH --ntasks=1                     # Run a single task
#SBATCH --cpus-per-task=8              # Request 4 CPUs per task (adjust as needed)
...
#SBATCH --gres-flags=enforce-binding   # Ensure all requested GPUs are on same node
#SBATCH --wait-all-nodes=1             # Wait until all resources are available
...
torchrun --nproc-per-node=2 ft_train_with_rl.py \
  ...
  --data-file "../../data/cot_atpg_data_v[1-2].csv" \
  --model-name llama-2 \
  ...
  --initial-ref-update-freq 100 \
  --final-ref-update-freq 100 \
  --wandb
