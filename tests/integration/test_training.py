...
def get_script_path():
    ...
    # tests/integration -> tests -> libatpgllm
    return Path(__file__).resolve().parents[2] / "scripts" / "train" / "training_code.py"

def get_repo_root():
    return Path(__file__).resolve().parents[2]

def get_sim_config_path():
    from atpgllm.training._paths import resolve_sim_config_path
    return resolve_sim_config_path()
...
class TestGRPOTraining:
    ...
    def test_grpo_training(self, output_dir, check_prerequisites):
        ...
        sim_config_path = get_sim_config_path()
...
try:
    import atpgllm.training
    from atpgllm.training.conversation import ConversationExample
    print("atpgllm.training imported successfully")
except ImportError as e:
    print(f"atpgllm.training import failed: {e}")
    raise SystemExit(1)
