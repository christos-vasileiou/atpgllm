...
> **Scope:** `DualAdapterGRPOTrainer` with `accelerate launch --multi_gpu` and a
> separate `trl vllm-serve` process for generation.
>
> **Paths (post-restructure):**
> - `scripts/train/run_training_code.sh` — GPU split, vLLM server, accelerate; **cwd = `libatpgllm/`**
> - `scripts/train/training_code.py` — dataset, model, GRPOConfig, trainer
> - `scripts/train/configs/*.conf` — sourced run configs (`OUTPUT_DIR=runs/…`)
> - `atpgllm/training/dual_adapter_grpo_trainer.py` — dual-adapter, tool loop, scoring
> - `atpgllm/training/reward_function_factory.py` + `atpgllm/llm/reward_funcs.py` — rewards
> - TRL `grpo_trainer.py` (upstream) — buffered generation, loss
>
> Shim: `tests/run_training_code.sh` → `scripts/train/run_training_code.sh`.
...
**Single-node** (`NUM_NODES=1`): last local GPU runs `trl vllm-serve`; remaining
GPUs are DDP training ranks. Example with `GPUS_PER_NODE=4`:

| GPU | Process | Role |
|-----|---------|------|
| #0 | `accelerate` rank 0 | Training + DDP coordination |
| #1 | `accelerate` rank 1 | Training |
| #2 | `accelerate` rank 2 | Training |
| #3 | `trl vllm-serve` | Inference-only generation |

vLLM starts on `VLLM_GPU=${GPU_ARRAY[-1]}` before training; `CUDA_VISIBLE_DEVICES`
is trimmed so trainers do not share that GPU. Configs with `GPUS_PER_NODE=2`
use 1 train + 1 vLLM. **Multi-node:** one whole node dedicated to vLLM
(`configs/grpo_h200_*node.conf`); see launcher comments.
...
`RewardFunctionFactory` → `test_generation_grpo_reward` (component dicts).
`DualAdapterGRPOTrainer` turns each dict into a train scalar via
`train_scalar_from_reward_components` (excludes `*_logonly`). Rewards are
gathered across ranks for group-relative GRPO normalisation.
...
Additionally, `max_model_len` was not being passed from the launcher through
`training_code.py` to the trainer.

**Fix:**
1. `scripts/train/run_training_code.sh` passes `--max_model_len`,
   `--max_completion_length`, and `--max_prompt_length`.
2. `scripts/train/training_code.py` forwards `vllm_max_model_len` to the trainer.
...
*Fixes live in `atpgllm/training/dual_adapter_grpo_trainer.py`. Paths above match
the post-restructure layout (`scripts/train/`, `atpgllm/training/`).*
