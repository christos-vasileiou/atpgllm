...
# /var/spool/slurmd/<job>/ (not writable); use the submit dir so
# launch-config snapshots land alongside jobs/ instead of failing to write.
# Prefer the real scripts/train location when SLURM_SUBMIT_DIR is the repo root.
if [ -n "${SLURM_SUBMIT_DIR:-}" ] && [ -f "${SLURM_SUBMIT_DIR}/training_code.py" ]; then
    _SCRIPT_DIR="$SLURM_SUBMIT_DIR"
elif [ -n "${SLURM_SUBMIT_DIR:-}" ] && [ -f "${SLURM_SUBMIT_DIR}/scripts/train/training_code.py" ]; then
    _SCRIPT_DIR="$(cd "${SLURM_SUBMIT_DIR}/scripts/train" && pwd)"
else
    _SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
fi
# libatpgllm repo root (scripts/train -> scripts -> repo)
_REPO_ROOT="$(cd "$_SCRIPT_DIR/../.." && pwd)"
_TRAIN_PY="$_SCRIPT_DIR/training_code.py"
_LAUNCH_CONFIG_DIR="$_REPO_ROOT/jobs/launch_configs"
mkdir -p "$_LAUNCH_CONFIG_DIR" "$_LAUNCH_CONFIG_DIR/snapshots" "$_REPO_ROOT/jobs" "$_REPO_ROOT/runs"
cd "$_REPO_ROOT"
# Slurm logs relative to repo root
export SLURM_LOG_FILE="${SLURM_LOG_FILE:-jobs/training_${SLURM_JOB_ID:-local}.out}"
export SLURM_ERROR_FILE="${SLURM_ERROR_FILE:-jobs/training_${SLURM_JOB_ID:-local}.err}"

CONFIG_ARG="${1:-}"
CONFIG_FILE="$CONFIG_ARG"
if [ -z "$CONFIG_FILE" ]; then
    echo "ERROR: no configuration file provided."
    echo "Usage: sbatch scripts/train/run_training_code.sh <config_file>"
    echo "   or: ./scripts/train/run_training_code.sh <config_file>"
    echo "SLURM (freeze at submit): ./scripts/train/submit_training_code.sh <config_file>"
    exit 1
fi
...
if [ -n "${RESUME_FROM:-}" ] && [ "$RESUME_FROM" != "None" ] && [ ! -d "$RESUME_FROM" ]; then
    ...
    elif [ -d "$_REPO_ROOT/tests/$RESUME_FROM" ]; then
        RESUME_FROM="$_REPO_ROOT/tests/$RESUME_FROM"
        echo "Resolved RESUME_FROM (legacy tests/) -> $RESUME_FROM"
    elif [ -d "$_SCRIPT_DIR/$RESUME_FROM" ]; then
...
run_multi_node() {
    ...
    echo "     --main_process_ip $head_ip --main_process_port $ACC_PORT $_TRAIN_PY ${CMD_ARGS[*]}"
    local rc=0
    if [ "$DRY_RUN" != "True" ]; then
