...
#
# Usage (from libatpgllm repo root or scripts/train):
#   ./scripts/train/submit_training_code.sh scripts/train/configs/grpo.conf
#   ./scripts/train/submit_training_code.sh configs/grpo.conf
#
# Direct / interactive runs (no queue delay) may use either:
#   ./scripts/train/run_training_code.sh configs/sft.conf
#   ./scripts/train/submit_training_code.sh configs/sft.conf   # also freezes at submit time
...
LAUNCH_CONFIG_DIR="$REPO_ROOT/jobs/launch_configs"
mkdir -p "$LAUNCH_CONFIG_DIR" "$REPO_ROOT/jobs" "$REPO_ROOT/runs"
...
# Submit from scripts/train so SLURM_SUBMIT_DIR resolves training_code.py locally.
cd "$SCRIPT_DIR"
exec sbatch --chdir="$REPO_ROOT" "${SBATCH_FLAGS[@]}" "$SCRIPT_DIR/run_training_code.sh" "$FROZEN"
