...
def resolve_merged_bf16_dir(adapter: Path) -> Path:
    ...
    code = (
        "from atpgllm.training.model_utils import export_qlora_merged_bf16; "
    ...
    if sampling_method in ("mcts", "evolutionary"):
        ...
        if budget is not None or best_of_n_width is not None:
            raise ValueError(
                ...
                f"{sampling_method} sampling method"
            )
        width = None
...
def sft_check_diversity(
    ...
    """
    from atpgllm.training.callbacks import INPUT_VECTOR_RE
...
Examples:
  ...
  # best_of_n: each of 20 completions is the best of 3 i.i.d. LLM samples
  python evaluate_model.py \\
  ...
  python evaluate_model.py \\
    ...
    args.config_path = resolve_sim_config_path(args.config_path)
