...
#   POLICY_FOLDER=grpo_7b_h100 ./eval_grpo_7b_policy_checkpoints.sh
#   POLICY_FOLDER=grpo_7b_h100/checkpoint-40 ./eval_grpo_7b_policy_checkpoints.sh
#   POLICY_FOLDER=grpo_7b_h100/checkpoint-40/policy ./eval_grpo_7b_policy_checkpoints.sh
#   EXP_ROOT=/path/to/grpo_7b_exper2 EVAL_RESULTS_DIR=/path/to/results ./eval_grpo_7b_policy_checkpoints.sh
#
# POLICY_FOLDER / EXP_ROOT path shapes (same three modes for both):
#   <exp>                         — discover and eval all checkpoint-* under that folder
#   <exp>/checkpoint-N            — eval that single checkpoint (resolve policy/ or combined/policy)
#   <exp>/checkpoint-N/policy     — eval that single checkpoint (adapter path given; also
#                                   accepts .../combined/policy)
...
#
# Examples:
#   SAMPLING_METHOD=best_of_n NUM_COMPLETIONS=32 BEST_OF_N_WIDTH=4 ./eval_grpo_7b_policy_checkpoints.sh
#   SAMPLING_METHOD=mcts NUM_COMPLETIONS=16 SEARCH_BUDGET=50 EVAL_RESULTS_DIR=./eval_results_grpo_7b_mcts ./eval_grpo_7b_policy_checkpoints.sh
#   DRY_RUN=1 POLICY_FOLDER=grpo_7b_h100/checkpoint-40/policy ./eval_grpo_7b_policy_checkpoints.sh
...
# Prefer runs/, fall back to legacy tests/ (POLICY_FOLDER may be exp, exp/ckpt, or exp/ckpt/policy).
EXP_ROOT="${EXP_ROOT:-}"
EXP_ROOT="${EXP_ROOT%/}"
if [ -z "$EXP_ROOT" ]; then
...
# lists checkpoint-* basenames (one entry for single-ckpt / policy paths).
CHECKPOINTS=()
_target_base="$(basename "$EXP_ROOT")"
if [[ "$_target_base" == "policy" ]]; then
  _parent="$(basename "$(dirname "$EXP_ROOT")")"
  if [[ "$_parent" == "combined" ]]; then
    _ckpt_dir="$(dirname "$(dirname "$EXP_ROOT")")"
  else
    _ckpt_dir="$(dirname "$EXP_ROOT")"
  fi
  _ckpt_name="$(basename "$_ckpt_dir")"
  if [[ ! "$_ckpt_name" =~ ^checkpoint- ]]; then
    echo "error: expected .../checkpoint-N/policy (or .../checkpoint-N/combined/policy), got: $EXP_ROOT" >&2
    exit 1
  fi
  EXP_ROOT="$(dirname "$_ckpt_dir")"
  CHECKPOINTS=("$_ckpt_name")
elif [[ "$_target_base" =~ ^checkpoint- ]]; then
...
unset _target_base _parent _ckpt_dir _ckpt_name
