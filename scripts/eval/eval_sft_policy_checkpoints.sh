#!/usr/bin/env bash
# Evaluate SFT LoRA checkpoints, regardless of base model family or size.
# Usage: ./eval_sft_policy_checkpoints.sh CHECKPOINT_OR_EXPERIMENT
...
# export CHECKPOINT=runs/sft_granite_4.2_8b/checkpoint-200
# ./scripts/eval/eval_sft_policy_checkpoints.sh "$CHECKPOINT"
# CHECKPOINT accepts a checkpoint or experiment directory; no default.
# An SFT checkpoint contains adapter_config.json directly.
...
usage() {
  ...
  echo "Evaluate SFT adapters; an experiment directory evaluates all checkpoint-* children."
}
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  usage
  exit 0
...
resolve_adapter() {
  [[ -f "$1/adapter_config.json" ]] || return 1
  echo "$1"
}

CHECKPOINTS=()
if resolve_adapter "$TARGET" >/dev/null || [[ "${TARGET##*/}" == checkpoint-* ]]; then
  CHECKPOINTS=("$TARGET")
...
for checkpoint in "${CHECKPOINTS[@]}"; do
  if ! adapter="$(resolve_adapter "$checkpoint")"; then
    [[ ${#CHECKPOINTS[@]} -gt 1 ]] || die "no SFT adapter_config.json at $checkpoint"
    echo "skip: no SFT adapter at $checkpoint" >&2
    continue
  fi
  name="${checkpoint##*/}"
  ...
  cmd=(
    ...
    --wandb_run_name "${WANDB_RUN_NAME:-${EXPERIMENT}_${name}_${SAMPLING_METHOD}}"
    "${SEARCH_ARGS[@]}"
  )
...
[[ "$evaluated" -gt 0 ]] || die "no evaluable SFT adapters under $TARGET"
